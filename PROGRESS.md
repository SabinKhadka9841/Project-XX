...
- **Tested on a phone**, since that's where students actually are. No sideways scrolling, and every button and link is now big enough to tap without hitting the wrong one — "Remove" was 16 pixels tall, which is a bad size for something that deletes. Email fields let a phone autofill the address rather than making someone type it.
